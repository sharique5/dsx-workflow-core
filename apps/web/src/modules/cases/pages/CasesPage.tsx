    ...
    <div className="px-4 sm:px-6 py-6 sm:py-8 max-w-7xl mx-auto">
      ...
      {matters && matters.length > 0 && (
        <div className="mb-4 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`sm:hidden shrink-0 rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
                  hasAdvancedFilters ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-slate-300 bg-white text-slate-600'
                }`}
                aria-label="Filters"
              >
          ...
          {/* Advanced filters */}
          <div className={`flex-wrap gap-2 ${showAdvanced ? 'flex' : 'hidden sm:flex'}`}>
      ...
      {!isLoading && !isError && filtered.length === 0 && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm text-center py-16">
          <p className="text-sm text-slate-400">
            {hasAnyFilter ? 'No cases match your filters.' : `No ${vocab.matter_plural.toLowerCase()} yet.`}
          </p>
          {!hasAnyFilter && (
